...
async function fetchAccounts(page: number, pageSize: number): Promise<{ data?: Promise<Paged<Account>>, error: boolean }> {
    ...
    return {
        data: response.json() as Promise<Paged<Account>>,
        error: false
    }
}
...
export default async function Page({ searchParams }: PageProps) {
    ...
    const { data, error } = await fetchAccounts(page, pageSize)
    ...
    if (!error) {
        ...
        accounts = payload?.details?.items ?? [];
